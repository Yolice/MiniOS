[GLOBAL idt_flush]
                      ;isr_handler和irq_handler分别是cpu和用户的中断处理函数入口，需要分开定义
idt_flush:
...
[EXTERN isr_handler]   ;isr_handler是0-31中cpu的中断处理函数
; 中断服务程序
isr_common_stub:
...
; 构造中断请求的宏
%macro IRQ 2                 ;利用模板构造通用的用户中断处理函数,模板参数有两个，%1和%2
[GLOBAL irq%1]
irq%1:
	cli
	push 0
...
[EXTERN irq_handler]         ;irq_handler是32-47中用户中断处理函数
irq_common_stub:
	pusha                    ; pushes edi, esi, ebp, esp, ebx, edx, ecx, eax
	...
	push esp             ;把中断号压入栈作为函数参数，调用该中断号的对应中断处理函数
	call irq_handler
	add esp, 4
